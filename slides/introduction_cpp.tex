...
\begin{frame}
\framedsl{Going Farther}
\end{frame}
\begin{frame}
\itemized{
	\item \textbf{OOP}: inheritance, virtual methods, abstract methods (\ctext{=0}), polymorphism
	\item \textbf{Types \& conversions}: implicit and explicit conversions, \ctext{static\_cast}, \ctext{dynamic\_cast}, \ctext{reinterpret\_cast}, \ctext{const} qualifier for variables and methods
	\item \textbf{C++3/11/14}: type inference with \ctext{auto}, lambdas (anonymous functions, functional programming)
}
\end{frame}
...
\newcommand{\resource}[2]{
  \includegraphics[height=1em]{figures/cpp-logo} \ \href{#1}{#2}
}
\begin{frame}
\sources{
